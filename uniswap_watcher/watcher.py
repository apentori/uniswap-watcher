...
def watch(config):
    ...
    psqlClient = PostgresClient(config.postgres.get("host"), config.postgres.get("port"), config.postgres.get("database"), config.postgres.get("username"), config.postgres.get("password"))
